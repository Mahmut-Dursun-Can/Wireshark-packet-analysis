...
def icmp_analyze(file_path,start_time, end_time):
    ...
    for packet in capture:
        if 'ICMP' in packet:  # ICMP protokolüne sahip paketleri kontrol et
            packet_count += 1
    ...
    # ICMP paketlerinin analizini yazdır
    print(f"Toplam ICMP paketi: {packet_count}")
    print("En çok ICMP paketi gönderen IP adresleri:")
    ...
    plt.title('En Çok ICMP Paket Gönderen IP Adresleri (Ping Flood)')
    plt.xlabel('IP Adresi')
    plt.ylabel('Gönderilen ICMP Paket Sayısı')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('png/icmp.png')
